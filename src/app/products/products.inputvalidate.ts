...
const productInputValidationSchema = z.object({
  ...
  description: z
    .string()
    .min(1, { message: 'Description name minimum 1 character' }),
  ...
  tags: z
    .array(z.string().min(1, { message: 'Tags name minimum 1 character' }))
    .min(1, { message: 'Tags array must contain at least 1 (string)element' }),
  variants: z
    .array(variantInputValidationSchema)
    .min(1, { message: 'Variants array must have product variant object' }),
